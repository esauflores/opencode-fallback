...
export function hashPart(mime: string, url: string): string {
  return crypto.createHash("sha256").update(`${mime}\0${url}`).digest("hex");
}
...
export function decodeDataUrl(
  ...
  if (estimatedDecodedBytes(payload) > maxBytes) return null;
  try {
...
function estimatedDecodedBytes(payload: string): number {
  let bytes = 0;
...
export function errorMessage(error: unknown): string {
  ...
  return text
    ...
    .replace(
      /\b(sk-[A-Za-z0-9_-]{8,}|sk-ant-[A-Za-z0-9_-]{8,}|AIza[A-Za-z0-9_-]{8,})\b/g,
      "[redacted]",
    )
    .replace(
